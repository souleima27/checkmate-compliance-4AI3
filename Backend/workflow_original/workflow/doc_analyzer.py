...
class ComplianceRuleManager:
    ...
    def _filter_applicable_rules(self) -> Dict[str, Dict]:
    ...
    def format_rules_for_llm(self) -> str:
        ...
        for rule_id, rule in self.applicable_rules.items():
            formatted.append(
                f"• {rule_id} [{rule.get('type', 'N/A')}]: {rule.get('description', 'N/A')}\n"
...
class DocumentComplianceAgent:
    ...
    def _load_rules_node(self, state: DocAnalyzerState) -> DocAnalyzerState:
        ...
        # Mise à jour RuleManager
        self.rule_manager.metadata = metadata or {}
        self.rule_manager.applicable_rules = self.rule_manager._filter_applicable_rules()
        
        # Hash pour cache (Règles structurelles)
        structural_rules = [r for r in self.rule_manager.all_rules.values() if r.get('categorie') == 'structurelle']
        rules_str = json.dumps(structural_rules, sort_keys=True)
...
{{
  ...
  "elements_conformes": [
    {{"element": "Titre du document", "location": "Slide 1 - slide_1_shape_1", "regle_id": "RC1", "justification": "..."}}
  ],
  "elements_non_conformes": [
    {{"element": "Disclaimer", "location": "Page 3", "regle_id": "RC2", "violation": "...", "gravite": "haute"}}
  ],
  "elements_manquants": [
    {{"element_requis": "...", "regle_id": "RC3"}}
  ],
  "recommandations": ["...", "..."]
}}
"""
